...
import { ComelitWebRTCAdapter } from '../ComelitWebRTCAdapter';

/**
 * Accessoire Sonnette/Caméra Comelit
 */
export class ComelitDoorbellAccessory implements CameraStreamingDelegate {
  private readonly doorbellService: Service;
  private readonly motionService: Service;
  private cameraController?: CameraController;
  private cameraId: string;
  private endpointId: string; // Endpoint complet depuis l'API
  
  private pendingSessions: Map<string, any> = new Map();
   ...
   */
  private async startStream(sessionId: string, request: any) {
    ...
    try {
      ...
      const ffmpegProcess = this.startFFmpegFromUDP(
        sessionInfo.response.address,
        sessionInfo.response.video.port,
        sessionInfo.response.video.srtp_key,
        sessionInfo.response.video.srtp_salt,
        sessionInfo.response.video.ssrc,
        video.width,
        video.height,
        video.fps,
      );
      ...
      const webrtcAdapter = new ComelitWebRTCAdapter(
        this.platform.log,
        this.platform.comelitAPI.getToken(),
      );
      ...
      const ffmpegProcess = this.startPlaceholderStream(
        sessionInfo.response.address,
        sessionInfo.response.video.port,
        sessionInfo.response.video.srtp_key,
        sessionInfo.response.video.srtp_salt,
   ...
   */
  private generateSDPOffer(sessionInfo: any): { sdp: string; videoPort: number } {
  ...
  private startFFmpegTranscoding(
    remoteIP: string,
    localPort: number,  // Port local pour recevoir les paquets RTP
    targetIP: string,
    targetPort: number,
    srtpKey: Buffer,
    srtpSalt: Buffer,
...
a=rtpmap:96 H264/90000`;
   ...
   */
  private async stopStream(sessionId: string) {
