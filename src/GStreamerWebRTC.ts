...
export interface GStreamerConfig {
  sessionId: string;
  localSdp: string;  // SDP Offer à envoyer à Comelit
  remoteSdp?: string;  // SDP Answer de Comelit (sera défini après la requête API)
  outputPort: number;  // Port UDP local pour sortir le flux RTP déchiffré
  outputHost?: string;  // Host (défaut: 127.0.0.1)
}
 ...
 */
export class GStreamerWebRTC extends EventEmitter {
