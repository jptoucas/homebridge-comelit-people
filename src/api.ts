...
export class ComelitAPI {
   ...
   */
  private buildEndpointId(type: string, deviceId: string): string {
   ...
   */
  private encodeEndpointId(endpointId: string): string {
   ...
   */
  async unlockDoor(endpointId: string) {
    const encodedId = this.encodeEndpointId(endpointId);
   ...
   */
  async startVideoSession(endpointId: string, sessionId: string, sdpOffer: string) {
    ...
    const response = await this.axios.put(
      `/devicecom/endpoint/${encodedId}/rtc/offer`,
      { 
        sessionId: sessionId,
        offer: sdpOffer 
      }
